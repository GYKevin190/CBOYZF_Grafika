...
void init_pong(Pong *pong, int width, int height)
{
    ...
    pong->leftplaya_score = 0;
    pong->rightplaya_score = 0;
}
...
void bounce_ball(Pong *pong)
{
    if (pong->ball.x - pong->ball.radius < 50)
    {
        ...
        else
        {
            ...
            pong->rightplaya_score += 1;
            printf("Bal jatekos: %d, Jobb jatekos: %d\n", pong->leftplaya_score, pong->rightplaya_score);
        }
    }
